...
public class BookServiceImpl implements BookService {
    ...
    private final AclServiceWrapperServiceImpl aclServiceWrapper;
    ...
    @Override
    public BookUpdateDto getBookUpdateDtoById(final Long id) {
        return bookRepository.findById(id)
                .map(bookMapper::bookToBookUpdateDto)
                .orElseThrow(() ->
                        new NotFoundException(
                                String.format("Не удалось получить книгу по Id для обновления: %d", id)));
    }
    ...
    public BookDto create(final BookCreateDto book) {
        ...
        final BookDto bookDto = bookMapper.bookToBookDto(bookRepository.save(bookToSave));
        aclServiceWrapper.createPermission(bookMapper.bookDtoToBookUpdateDto(bookDto));
        return bookDto;
    }
